...
const safeFileName = (value) => {
    const base = String(value || 'storage_maintenance_log');
    return base
        .replace(/[^a-z0-9_-]+/gi, '_')
        .replace(/^_+|_+$/g, '')
        .toLowerCase() || 'storage_maintenance_log';
};
...
const buildChecklistTable = (details) => {
    ...
    const filteredItems = details.checklist_items.filter((item) => {
        ...
        return item.section !== 'battery_inspection' && item.section !== 'fault_codes';
    });
    ...
    let lastSection = '';
    const hasImages = sorted.some((row) => row.imageUrl && String(row.imageUrl).startsWith('https://'));
...
const createStorageMaintenancePdf = async ({
    details,
    unitInfoData,
    remarks,
    defectRemarks,
    ...
    const headerTitle = 'Indo Traktor Form Storage Maintenance';
    const titleSize = 14;
    ...
    const shouldWrapValue = (label) => label === 'Transmission Type';
    ...
    let renaultExtrasPageStarted = false;
    const startRenaultExtrasPage = () => {
        if (renaultExtrasPageStarted) return;
        startNewPage();
        renaultExtrasPageStarted = true;
    };

    if (brandLower === 'renault') {
        const allItems = details.checklist_items || [];
        const batteryData = allItems.filter((item) => item.section === 'battery_inspection');
        const faultCodesData = allItems.filter((item) => item.section === 'fault_codes');

        const getItem = (name) => batteryData.find((item) => item.itemName === name) || {};
        ...
        if (batteryData.length > 0) {
            startRenaultExtrasPage();
            drawSectionTitle('Battery Inspection (Renault) - Measurements', 20);
            await drawTable(
                [
                    { key: 'section', label: 'Section', width: 100 },
                    { key: 'itemName', label: 'Item Name', width: 120 },
                    { key: 'electrolyte', label: 'Electrolyte Level', width: 120 },
                    { key: 'voltage', label: 'Voltage', width: 80 },
        ...
        if (faultCodesData.length > 0) {
            const faultRows = faultCodesData
            ...
            startRenaultExtrasPage();
            drawSectionTitle('Fault Codes (Renault) - Diagnostics', 20);
            await drawTable(
                [
                    { key: 'section', label: 'Section', width: 120 },
                    { key: 'code', label: 'Fault Code', width: 140 },
    ...
    if (brandLower === 'sdlg' && defectRemarks && defectRemarks.length > 0) {
        ...
        drawSectionTitle('Defects & Corrective Actions', 26);
        await drawTable(
            [
...
const LogData = ({ title, apiUrl }) => {
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            const bytes = await createStorageMaintenancePdf({
            ...
            const fileBase = safeFileName(
                `Storage Maintenance - ${brandLabel || 'brand'} - ${vinValue || 'vin'}`
            );
